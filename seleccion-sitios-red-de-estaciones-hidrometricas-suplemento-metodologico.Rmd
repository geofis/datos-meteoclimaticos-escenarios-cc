...
ap <- sapply(escenario, function(x)
  obj_sf <- st_read(paste0(dem_proc_dir, 'areas_prospeccion_', x, '_km2.gpkg')),
  simplify = F)
prioridad <- st_read(paste0(dem_proc_dir, 'prioridad_reescalada.gpkg'))
```
...
map(escenario, function(x) resultados_por_area[[x]] %>%
      ...
      caption = paste0('Número de tramos identificados por cada área de prospección ',
                       'según nivel de prioridad y densidad. Escenario de densidad: ', x, ' kilómetros cuadrados')) %>%
      kable_styling(bootstrap_options = c("hover", "condensed"), full_width = T))
# mapas <- sapply(
