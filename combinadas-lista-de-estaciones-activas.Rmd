...
reddom_sf <- st_read('out/consolidado_estaciones_sf.gpkg') %>% 
  ...
  select(Nombre = idOK) %>% mutate(Estado = 'activa') %>% 
  mutate(Longtiud = st_coordinates(geom)[,1], Latitud = st_coordinates(geom)[,2])
reddom_sf %>% st_write('con_indicacion_estatus_reddom_climatica.gpkg')
```
