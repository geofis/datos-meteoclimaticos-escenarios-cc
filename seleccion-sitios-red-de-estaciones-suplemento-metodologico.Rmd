...
variables_eng <- c(
    acce = "distance to access routes",
    temp = "thermal seasonality",
    pluv = "rainfall seasonality",
    habi = "habitat heterogeneity",
    agua = "distance to water bodies",
    pend = "slope",
    inso = "hours of direct sunshine",
    elev = "elevation"
)
```
...
prefagg_eng <- flujo_completo_ahp$aggpref %>%
  as.data.frame() %>% 
  rename(`Aggregated Preferences` = AggPref, `Standard Deviation` = SD.AggPref) %>% 
  rownames_to_column('Variable') %>% 
  mutate(Variable = factor(Variable, labels = variables_eng[sort(names(variables_eng))])) %>% 
  arrange(desc(`Aggregated Preferences`))
...
```{r umbralesapuntuaciones_eng}
puntuaciones_umbrales_eng <- map(objetos, function(x) get(x)[['intervalos_y_etiquetas']] %>% 
  pivot_longer(cols = -matches('puntuación|etiquetas'), names_to = 'criterio') %>%
  mutate(criterio = gsub(' intervalos', '', criterio)) %>% 
  group_by(across(all_of(matches('etiquetas|criterio')))) %>% 
  summarise(value = paste(value, collapse = ' y ')) %>% 
  ...
  select(criterio, rev(fuente))) %>%
  bind_rows() %>% 
  mutate(criterio = factor(criterio, levels = variables, labels = variables_eng))
colnames(puntuaciones_umbrales_eng)[1:5] <- c(
  'variable',
  fuente_eng[match(colnames(puntuaciones_umbrales_eng)[2:5], fuente)])
```
...
# En inglés
areas_proporcionales_eng <- areas_proporcionales
areas_proporcionales_eng$criterio <- variables_eng[match(areas_proporcionales$criterio, variables)]
colnames(areas_proporcionales_eng)[2:5] <- fuente_eng[match(colnames(areas_proporcionales)[2:5], fuente)]
colnames(areas_proporcionales_eng)[1] <- 'Variable'
colnames(areas_proporcionales_eng)[6] <- "Total"
```
...
all_criteria_mapa <- all_criteria %>%
  ...
  mutate(value = factor(value, levels = rev(fuente))) %>% 
  na.omit() %>% 
  ggplot +
  aes(fill = value) +
  geom_sf(lwd=0) + 
...
```{r}
paleta_eng <- c("#018571", "#80cdc1", "#dfd2b3", "#a6611a")
names(paleta_eng) <- rev(fuente_eng)
all_criteria_mapa_eng <- all_criteria %>%
  select(all_of(contains('etiquetas'))) %>% 
  rename_with(~ stringr::str_replace(.x, 
                                       pattern = ' etiquetas', 
                                       replacement = ''), 
                matches('etiquetas')) %>% 
  pivot_longer(cols = -geometry) %>% 
  mutate(name = variables_eng[names(variables)[match(name, variables)]]) %>%
  mutate(value = factor(fuente_eng[names(fuente_eng)[match(value, fuente)]], levels = rev(fuente_eng))) %>% 
  na.omit() %>% 
  ggplot +
  aes(fill = value) +
  geom_sf(lwd=0) + 
  scale_fill_manual(values = paleta_eng) +
  labs(title = paste('Reclassification of criteria values')) +
  geom_sf(data = pais, fill = 'transparent', lwd = 0.5, color = 'grey50') +
  facet_wrap(~ name, ncol = 2) +
  theme_bw() +
  theme(
...
```{r}
# Ambos mapas, sin y con exclusión por factores limitantes, en inglés
all_criteria_scores_non_excluded_and_excluded_map_eng <- all_criteria_scores %>%
  mutate(Type = 'Aggregated categories') %>% 
  select(hex_id, Type, `Aggregated categories` = `Categoría agregada`) %>% 
  bind_rows(
    all_criteria_scores_excluded %>% 
      mutate(Type = 'Aggregated categories, with exclusion') %>% 
      select(hex_id, Type, `Aggregated categories` = `Categoría agregada`)
  ) %>% 
  mutate(
    `Aggregated categories` = factor(
      fuente_eng[names(fuente_eng)[match(`Aggregated categories`, fuente)]], levels = rev(fuente_eng))) %>% 
  ggplot +
  aes(fill = `Aggregated categories`) +
  geom_sf(lwd=0) + 
  scale_fill_manual(values = paleta_eng) +
  # labs(title = paste('Aggregated criteria')) +
  geom_sf(data = pais, fill = 'transparent', lwd = 0.5, color = 'grey50') +
  facet_wrap(~ Type) +
  theme_bw() +
  theme(
...
```{r}
# Ambas tablas de áreas proporcionales según categorías, sin y con exclusión por factores limitantes, en español e inglés
areas_prop_all_criteria_con_sin_excl_esp <- areas_proporcionales_all_criteria %>%
  inner_join(
    y = areas_proporcionales_all_criteria_excluded,
    by = 'Categoría agregada', 
    suffix = c(' (sin exlusion)', ' (con exclusión)'))
areas_prop_all_criteria_con_sin_excl_eng <- areas_prop_all_criteria_con_sin_excl_esp %>%
  rename(`Aggregated category` = `Categoría agregada`) %>%
  rename_with(function(colname) {
    colname %>%
      gsub("proporción", "Proportion", .) %>%
      gsub("sin exlusion", "without exclusion", .) %>%
      gsub("con exclusión", "with exclusion", .)}) %>% 
  mutate(
    `Aggregated category` = factor(
      fuente_eng[names(fuente_eng)[match(`Aggregated category`, fuente)]], levels = rev(fuente_eng))) %>% 
  mutate(`Aggregated category` = gsub("^(.)", "\\U\\1", `Aggregated category`, perl = TRUE))
```
...
Igualmente, siguiendo con la red del INDRHI, incluyendo en este caso las estaciones en estado "Regular", notaremos que igualmente las distancias de separación entre estaciones siguen siendo muy por encima del mínimo recomendado por la OMM.
...
Las estaciones de la red de ONAMET fueron caracterizadas recientemente en dos estados "activa" e "inactiva o no reportada". En nuestro análisis de vecindad para eliminar redundancia, consideramos únicamente las activas. Por otra parte, la red del INDRHI cuenta con estaciones que, en 2019 se encontraban en estado "Bueno", "Regular" y "Malo". Decidimos considerar tanto las estaciones en estado "Bueno" como las que se encontraban en estado "Regular". Así, para cada uno de nuestros escenarios de densidad (100, 150 y 250 km<sup>2</sup>), eliminamos redundancia usando dos redes combinadas distintas: 1) ONAMET activas + INDRHI buenas, y 2) ONAMET activas + INDRHI buenas y regulares. De esta manera, como producto final, obtuvimos 6 escenarios distintos.
...
actbue_r <- rasterize(x = as(actbue, 'Spatial'),
                                      ...
                                      field = 1)
actbue_d <- raster::distance(actbue_r)
...
actbuereg_r <- rasterize(x = as(actbuereg, 'Spatial'),
                                      y = raster(extent(ind_esp),
                                                 resolution = 500, crs = 'EPSG:32619'),
                                      field = 1)
actbuereg_d <- raster::distance(actbuereg_r)
actbuereg_d %>% writeRaster('out/onamet_indrhi_actbuereg_dist_500x500_distancia.tif',
                            overwrite = T, setStatistics = F)
```

En el caso del escenario "100 km<sup>2</sup> por estación", la distancia de corte entre estaciones es de 11 km. Por lo tanto, eliminamos las estaciones propuestas por nosotros que caían dentro de ese rango respecto de estaciones de ONAMET y/o INDRHI existentes.

En nuestros análisis, usamos igualmente las estaciones de la red privada, pero sólo para fines de representación. Nuestro objetivo al mostrar la red privada sobre los distintos escenarios de densidad, es visibilizar la oportunidad de colaboración y posible sinergia para contribuir a la mejora de la calidad del dato.
...
```{r}
# Mapas de todos los escenarios con las buenas, sin privadas, en inglés
# Escenario 100 km2
{esc_100_activas_buenas_mapa_eng <- esc_100_activas_buenas_mapa +
  ggtitle(NULL)
esc_100_activas_buenas_mapa_eng$layers <- esc_100_activas_buenas_mapa_eng$layers[-3]
esc_100_activas_buenas_mapa_eng$data$id <- factor(
  x = esc_100_activas_buenas_mapa_eng$data$id,
  labels = c('Existing stations', 'Proposed sites'))
esc_100_activas_buenas_mapa_eng$data <- esc_100_activas_buenas_mapa_eng$data %>%
  st_join(
    y = all_criteria_scores_excluded %>% select(Category = `Categoría agregada`),
    join = st_nearest_feature) %>% 
  mutate(Category = factor(
    x = fuente_eng[names(fuente_eng)[match(Category, fuente)]],
    levels = rev(fuente_eng)))
esc_100_activas_buenas_mapa_eng <- ggplot() +
  geom_sf(data = pais, fill = 'transparent', color = 'grey50') +
  geom_sf(
    data = esc_100_activas_buenas_mapa_eng$data %>% filter(id == 'Existing stations'),
    alpha = 0.8, fill = 'grey', aes(shape = id), size = 2.5) +
  geom_sf(
    data = esc_100_activas_buenas_mapa_eng$data %>% filter(id == 'Proposed sites') %>% 
      group_by(Category) %>% mutate(n = n(), Category = paste0(Category, ' (', n, ')')),
    alpha = 0.8, aes(fill = Category), shape = 21, size = 2.5) +
  scale_fill_manual(
    values = c("#018571", "#80cdc1")) +
  scale_shape_manual(
    values = c(25, 21),
    guide = guide_legend(title = NULL, order = 2, override.aes = list(fill = "grey"))) +
  labs(fill = "Proposed sites") +
  ggspatial::annotation_scale(style = 'ticks', height = unit(0.15, "cm"), location = 'br') +
  theme_bw()
# Escenario 150 km2
esc_150_activas_buenas_mapa_eng <- esc_150_activas_buenas_mapa +
  ggtitle(NULL)
esc_150_activas_buenas_mapa_eng$layers <- esc_150_activas_buenas_mapa_eng$layers[-3]
esc_150_activas_buenas_mapa_eng$data$id <- factor(
  x = esc_150_activas_buenas_mapa_eng$data$id,
  labels = c('Existing stations', 'Proposed sites'))
esc_150_activas_buenas_mapa_eng$data <- esc_150_activas_buenas_mapa_eng$data %>%
  st_join(
    y = all_criteria_scores_excluded %>% select(Category = `Categoría agregada`),
    join = st_nearest_feature) %>% 
  mutate(Category = factor(
    x = fuente_eng[names(fuente_eng)[match(Category, fuente)]],
    levels = rev(fuente_eng)))
esc_150_activas_buenas_mapa_eng <- ggplot() +
  geom_sf(data = pais, fill = 'transparent', color = 'grey50') +
  geom_sf(
    data = esc_150_activas_buenas_mapa_eng$data %>% filter(id == 'Existing stations'),
    alpha = 0.8, fill = 'grey', aes(shape = id), size = 2.5) +
  geom_sf(
    data = esc_150_activas_buenas_mapa_eng$data %>% filter(id == 'Proposed sites') %>% 
      group_by(Category) %>% mutate(n = n(), Category = paste0(Category, ' (', n, ')')),
    alpha = 0.8, aes(fill = Category), shape = 21, size = 2.5) +
  scale_fill_manual(
    values = c("#018571", "#80cdc1")) +
  ...
  theme_bw()
# Escenario 250 km2
esc_250_activas_buenas_mapa_eng <- esc_250_activas_buenas_mapa +
  ggtitle(NULL)
esc_250_activas_buenas_mapa_eng$layers <- esc_250_activas_buenas_mapa_eng$layers[-3]
esc_250_activas_buenas_mapa_eng$data$id <- factor(
  x = esc_250_activas_buenas_mapa_eng$data$id,
  labels = c('Existing stations', 'Proposed sites'))
esc_250_activas_buenas_mapa_eng$data <- esc_250_activas_buenas_mapa_eng$data %>%
  st_join(
    y = all_criteria_scores_excluded %>% select(Category = `Categoría agregada`),
    join = st_nearest_feature) %>% 
  mutate(Category = factor(
    x = fuente_eng[names(fuente_eng)[match(Category, fuente)]],
    levels = rev(fuente_eng)))
esc_250_activas_buenas_mapa_eng <- ggplot() +
  geom_sf(data = pais, fill = 'transparent', color = 'grey50') +
  geom_sf(
    data = esc_250_activas_buenas_mapa_eng$data %>% filter(id == 'Existing stations'),
    alpha = 0.8, fill = 'grey', aes(shape = id), size = 2.5) +
  geom_sf(
    data = esc_250_activas_buenas_mapa_eng$data %>% filter(id == 'Proposed sites') %>% 
      group_by(Category) %>% mutate(n = n(), Category = paste0(Category, ' (', n, ')')),
    alpha = 0.8, aes(fill = Category), shape = 21, size = 2.5) +
  scale_fill_manual(
    values = c("#018571", "#80cdc1")) +
...
```{r}
save.image('r-image-site-selection.RData')
sessionInfo()
...
#  [1] spdep_1.3-5      spData_2.3.1     sf_1.0-12        janitor_2.2.0    ahpsurvey_0.4.1 
#  [6] lubridate_1.9.3  forcats_1.0.0    stringr_1.5.0    dplyr_1.1.4      purrr_1.0.1     
# [11] readr_2.1.4      tidyr_1.3.0      tibble_3.2.1     ggplot2_3.5.1    tidyverse_2.0.0 
# [16] kableExtra_1.3.4 psych_2.3.3      raster_3.6-26    sp_2.1-4        
# 
# loaded via a namespace (and not attached):
#  [1] tidyselect_1.2.0   viridisLite_0.4.2  farver_2.1.1       fastmap_1.1.1      digest_0.6.31     
#  [6] timechange_0.2.0   lifecycle_1.0.3    ggspatial_1.1.8    terra_1.7-78       dbscan_1.1-12     
# [11] magrittr_2.0.3     compiler_4.3.0     rlang_1.1.4        sass_0.4.6         tools_4.3.0       
# [16] yaml_2.3.7         utf8_1.2.3         knitr_1.42         labeling_0.4.2     bit_4.0.5         
# [21] classInt_0.4-9     mnormt_2.1.1       xml2_1.3.4         KernSmooth_2.23-20 withr_2.5.0       
# [26] grid_4.3.0         fansi_1.0.4        e1071_1.7-13       colorspace_2.1-0   scales_1.3.0      
# [31] cli_3.6.1          rmarkdown_2.21     crayon_1.5.2       generics_0.1.3     rstudioapi_0.14   
# [36] httr_1.4.6         tzdb_0.4.0         DBI_1.1.3          cachem_1.0.8       proxy_0.4-27      
# [41] readODS_1.8.0      rvest_1.0.3        parallel_4.3.0     s2_1.1.4           vctrs_0.6.5       
# [46] boot_1.3-28        webshot_0.5.4      jsonlite_1.8.4     hms_1.1.3          bit64_4.0.5       
# [51] systemfonts_1.0.4  jquerylib_0.1.4    units_0.8-2        rgdal_1.6-6        glue_1.6.2        
# [56] codetools_0.2-18   stringi_1.7.12     gtable_0.3.5       deldir_1.0-9       munsell_0.5.0     
# [61] pillar_1.9.0       htmltools_0.5.5    R6_2.5.1           wk_0.7.3           Rdpack_2.4        
# [66] vroom_1.6.3        evaluate_0.21      lattice_0.20-45    rbibutils_2.2.13   snakecase_0.11.0  
# [71] bslib_0.4.2        class_7.3-20       Rcpp_1.0.10        svglite_2.1.1      nlme_3.1-155      
