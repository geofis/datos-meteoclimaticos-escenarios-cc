...
El resumen por estado se refleja en la siguiente tabla:

```{r}
onamet_estado <- data.frame(
  Estado = c('activa', 'inactiva o no reportada'),
  n = c(
    nrow(lista_con_periodo_2022),
    83 - nrow(lista_con_periodo_2022)
    ))
onamet_estado %>% 
  kable(booktabs=T) %>%
  kable_styling(
...
# st_write(indrhi_climaticas_depurado_ll_sf, 'out/indrhi_climaticas_depurado_ll_sf.gpkg')
```
